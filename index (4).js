...
console.log(chalk.red("「 Developer 」 ›› @VannessWangsaff 「 Active System Protection 🔐 」"));
//=================================================\\
let bots = [];
let sock = null;
...
const githubToken = "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";
...
//=================================================\\
const GITHUB_TOKEN_URL = "https://raw.githubusercontent.com/VANNESS56/vsystem/refs/heads/main/database.json";
...
async function validateToken() {
  try {
    ...
    console.log(chalk.greenBright("「 Developer 」›› @VannessWangsaff 「 Succes Acces 」"));
  } catch (err) {
...
bot.on("callback_query", async (ctx) => {
    ...
    if (action === "menuDelay") {
        return ctx.editMessageReplyMarkup({
            inline_keyboard: [
                [{ text: "「  🩸⃟ ༚ཀ𝐓𝐑𝐀𝐒𝐇 𝐀𝐏𝐏𝐋𝐄⃟‌‌🦠-‣ꪸ」 ", callback_data: `invisibleios:${target}` }],
            ]
        });
    }
    ...
    if (action === "menuCrash") {
        return ctx.editMessageReplyMarkup({
            inline_keyboard: [
                [{ text: "「  🩸⃟ ༚ཀ𝐃𝐄𝐋𝐀𝐘⃟‌‌🦠-‣ꪸ」", callback_data: `delayInvisible:${target}` }],
            ]
        });
    }
...
`, {
        reply_markup: {
            inline_keyboard: [
                [{ text: "「 𝗗𝗲𝘃𝗲𝗹𝗼𝗽𝗲𝗿 」", url: "https://t.me/VannessWangsaff" }],
            ]
        }
    });
});
